...
app.component('v-select',{
		...
		filtered(){
			...
			// if(this.options){
				filtered = options.filter(opt=>{
					var value = this.keyvalue?opt[this.keyvalue.split('.').pop()]:opt;
		...
		selectedObjects(){
			...
			if(this.keyid){
				this.selected = options.filter(opt=>{
					var id=this.keyid?opt[this.keyid.split('.').pop()]:opt;
					var selected = Array.isArray(this.text)?this.text:[this.text];
					// console.log(selected,id);
					return selected.includes(id)
				})
				var selected = this.selected.map(opt=>opt[this.keyvalue.split('.').pop()]);
				if(this.isMultiple){
					return selected;
				}else{
	...
	template:`
		<div class="relative">
			...
			<div v-if="isLoading" style="position:absolute;right:1em;z-index:1">
				<i class="white extra-large feather-loader"></i>
			</div>
